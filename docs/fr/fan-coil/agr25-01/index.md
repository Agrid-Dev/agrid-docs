# AGR25-01 — Thermostat pour ventilo-convecteurs

<div class="download-buttons" markdown>
[Télécharger le manuel d'installation (PDF)](assets/Complete_Manual_AGRID_AGR25-01_FR.pdf){ .md-button .md-button--primary }
[Télécharger les schémas de câblage (PDF)](assets/Wiring_Diagrams_AGRID_FR.pdf){ .md-button }
</div>

## Spécifications techniques

| Paramètre | Valeur |
|---|---|
| Produit | Thermostat pour ventilo-convecteurs |
| Modèle | AGR25-01 |
| Alimentation | 220-240V~ 50Hz |
| Puissance consommée | MAX 2W |
| Affichage | Écran tactile couleur 4 pouces |
| Communication | Wi-Fi 2,4 GHz IEEE 802.11 b/g/n/ax <br> Bande : 2 400 – 2 483,5 MHz <br> Puissance RF max : 19,81 dBm EIRP (≤ 100 mW) |
| Protection | Classe II (double isolation), IP20 |
| Température de fonctionnement | +10°C à +50°C |
| Plage de consigne | 5°C à 35°C (configurable) |
| Précision | ± 1°C |
| Dimensions | 105 × 85 × 30 mm |
| Poids | 196 g |
| Montage | Fixation murale en saillie |

## Sorties et entrées

| Composant | Spécification |
|---|---|
| Sorties relais | 5 relais SPST-NO (RL1–RL5), Max 5A (Ind.) 220-240V~ |
| Sorties 0-10V | 3 sorties DAC (DAC1, DAC2, DAC3) |
| Entrées capteurs externes | 2 entrées (S1, S2) |
| Capteurs intégrés | Température, humidité, luminosité |

## Action et cycles

| Paramètre | Valeur |
|---|---|
| Type d'action | Type 1, micro-coupure (Type 1.B) |
| Cycles automatiques | 100 000 |
| Cycles manuels | 6 000 |

## Conditions d'environnement

| Paramètre | Valeur |
|---|---|
| Environnement | Intérieur sec uniquement |
| Altitude maximale | ≤ 2 000 m |
| Humidité relative | 5–95% sans condensation |
| Catégorie de surtension | II |
| Degré de pollution | 2 |

## Symboles

| Symbole | Description |
|:---:|---|
| ![CE](../../../assets/ce.png){ .cert-icon } | Conformité européenne (directives RED, LVD, EMC, RoHS, ErP, DEEE) |
| ![Classe II](../../../assets/class2.png){ .cert-icon } | Appareil à double isolation — pas de terre de protection |
| ~ | Courant alternatif (AC) |
| ⎓ | Courant continu (DC) |
| T50 | Température ambiante maximale de fonctionnement : 50°C |
| IP20 | Protégé contre les corps solides > 12,5 mm — pas de protection contre l'eau |
| Type 1.B / µ | Action automatique Type 1, micro-coupure (un seul pôle commuté) |
| ![DEEE](../../../assets/weee.png){ .cert-icon } | Ne pas jeter avec les déchets ménagers (voir [Mise au rebut](installation.md#mise-au-rebut)) |
| ![RoHS](../../../assets/rohs.png){ .cert-icon } | Limitation des substances dangereuses (directive 2011/65/UE) |

## Déclaration UE de conformité

Déclaration UE de conformité simplifiée (conformément à l'article 10, paragraphe 9, de la directive 2014/53/UE) :

AGRID SAS déclare par la présente que le thermostat AGR25-01 est conforme aux directives et règlements européens suivants :

- Directive 2014/35/UE (DBT) — Sécurité des équipements électriques basse tension
- Directive 2014/30/UE (CEM) — Compatibilité électromagnétique
- Directive 2014/53/UE (RED) — Équipements radioélectriques
- Directive 2011/65/UE (RoHS) — Limitation des substances dangereuses
- Directive 2009/125/CE (ErP) — Écoconception des produits liés à l'énergie
- Règlement (UE) 2024/1103 — Écoconception des dispositifs de chauffage des locaux et des dispositifs de régulation distincts
- Règlement délégué (UE) 2022/30 — Exigences en matière de cybersécurité (article 3, paragraphe 3, de la directive RED)
- Directive 2012/19/UE (DEEE) — Déchets d'équipements électriques et électroniques
- Règlement (CE) n° 1907/2006 (REACH) — Enregistrement, évaluation, autorisation et restriction des substances chimiques
- Recommandation du Conseil 1999/519/CE — Limitation de l'exposition du public aux champs électromagnétiques

Normes harmonisées appliquées : EN 60730-1:2011, EN 60730-2-9:2010, EN 62311:2008, EN 55014-1:2017/A11:2020, EN 55014-2:2015, EN 61000-3-2:2019, EN 61000-3-3:2013/A1:2019, EN 301489-1 V2.2.3, EN 301489-17 V3.3.1, EN 300328 V2.2.2, EN 18031-X, EN 50581:2012.

Le texte complet de la déclaration UE de conformité est disponible sur demande auprès de l'importateur (voir Contact ci-dessous).

## Contact

**Fabricant :** Beijing Breeze Technology Co., Ltd. — Floor 22, Bldg 1, Yard 1, Guanyinan South St., Tongzhou Dist., Beijing, China

**Importateur :** AGRID SAS — 33 rue du Faubourg Saint-Antoine, 75011 Paris, FRANCE — Email : contact@a-grid.com
