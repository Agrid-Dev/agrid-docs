...
Le tableau compte **264 entrées**, dont **10** encore au stade planification (signalées _(non implantée)_ dans la colonne « Description »).
...
- **Real-Time Variables**
  ...
  - [Internal Regulation Resistance Status](#internal-regulation-resistance-status)
...
| `Deferred_Command_Count` | `int32` | r | 0 | Nombre de commandes MQTT reçues avec un paramètre `delay` et pas encore exécutées (cf. README section MQTT « Report différé des commandes »), toutes files confondues : de 0 à 7 (5 cases pour les commandes sans chaîne, 2 pour les transferts, scripts et écritures d'une chaîne). Une file pleine refuse la commande. Publiée à chaque changement ; revient à 0 à l'exécution de la dernière, sur `CANCEL_DELAYED` et au reboot (les commandes en attente ne survivent pas à un redémarrage). Lecture seule. |
...
| `Event` | `enum` | — | NO_EVENT | Dernier événement système remonté par le firmware : boot, Wi-Fi, MQTT, script, téléchargement OTA, opérations flash et actions de configuration.<br>_123 valeurs admises — les lister via `GET /enums`._ |
...
### Internal Regulation Resistance Status

| Variable | Type | Accès | Défaut | Description |
|---|---|---|---|---|
| `Resistance_Usage_Effective` | `enum` | r | RESISTANCE_USAGE_SERIAL | Usage de la résistance réellement appliqué par la régulation à l'itération courante. Vaut `Resistance_Usage_Type` tant qu'une source d'eau chaude peut atteindre la batterie ; vaut `RESISTANCE_USAGE_PARALLEL` dès qu'aucune ne le peut, la résistance étant alors seule à pouvoir chauffer. Sans résistance à bord (types sans `_2WIRES`), vaut l'usage configuré : il n'y a rien à appliquer, et rien n'est publié. Deux cas de substitution : un type `COOL_ONLY` (aucune eau chaude n'y arrive jamais, quel que soit le changeover) ou un type dont l'eau suit le changeover alors que celui-ci est en COOL.<br>**Ne dépend PAS de `Allow_Res_Heat_ChangeOver_COOL`** : ce drapeau gouverne uniquement l'admissibilité du mode HEAT, jamais le régime de la résistance. Conséquence à connaître : un `2T_COOL_ONLY_2WIRES` configuré en SERIAL est en PARALLEL effectif toute l'année, drapeau ou pas.<br>Recalculé à chaque itération et publié sur changement : permet à la GTB et au banc de distinguer « SERIAL configuré » de « PARALLEL effectif ». Lecture seule.<br>_Valeurs : RESISTANCE_USAGE_PARALLEL · RESISTANCE_USAGE_SERIAL_<br>_Absente du build MINIMAL._ |
...
| `RSSI` | `int32` | r | 0 | Force de la connexion Wifi en dBm, relue toutes les 60 s ; `-2147483648` quand la station n'est pas associée. Journalisée à la première mesure d'une association, puis à chaque écart d'au moins 6 dB avec la dernière valeur journalisée (seuil compilé `WIFI_RSSI_LOG_DELTA_DB`) — en dessous, c'est la fluctuation normale d'un lien stable. Un changement de borne rouvre la série. |
| `Wifi_AP_MAC` | `char` | r | — | Adresse MAC (BSSID) du point d'accès Wi-Fi auquel le thermostat est associé, au format `AABBCCDDEEFF` (sans séparateur) ; vide si non associé. Rafraîchie en même temps que `RSSI` (et à chaque contrôle de roaming) : permet de savoir sur quel AP d'un même SSID le thermostat est connecté. Journalisée à chaque changement de borne — sans seuil, c'est une identité et non une mesure : c'est elle qui rend la série `RSSI` lisible, une chute de 20 dB ne voulant pas dire la même chose selon que la borne a changé ou non. Lecture seule. |
...
| `Wifi_Roaming_RSSI_Threshold` | `int32` | rw | -67 | Exprimé en dBm. Seuil de roaming : tant que le RSSI de l'AP courant reste au-dessus, rien ne se passe. S'il passe strictement en dessous, le thermostat scanne son SSID toutes les `Wifi_Roaming_Delay` secondes et bascule sur un autre point d'accès du même SSID reçu au moins `Wifi_Roaming_RSSI_Hysteresis` dB plus fort. La bascule coupe brièvement la connexion (quelques secondes, MQTT se reconnecte). Elle n'est pas tentée pendant un transfert (upload, download) ni pendant un script. Sous ce seuil, les scripts sont refusés (event `EVENT_SCRIPT_LOW_RSSI`). Un point d'accès vers lequel la bascule a échoué est écarté pendant 10 périodes (event `EVENT_WIFI_ROAMING_FAILED`).<br>_Plage : -100 - -30_ |
| `Wifi_Roaming_Delay` | `int32` | rw | 300 | Exprimé en secondes. Période du roaming Wi-Fi : relecture du RSSI et, s'il est sous `Wifi_Roaming_RSSI_Threshold`, scan du SSID courant à la recherche d'un meilleur point d'accès. 0 = roaming désactivé. Chaque scan interrompt brièvement le trafic Wi-Fi.<br>_Plage : 0 (désactivé) ou 30 - 86400_ |
| `Wifi_Roaming_RSSI_Hysteresis` | `int32` | rw | 10 | Exprimé en dB. Hystérésis du roaming Wi-Fi : quand le RSSI est sous `Wifi_Roaming_RSSI_Threshold`, le thermostat ne bascule vers un autre point d'accès du même SSID que s'il est reçu au moins cette valeur au-dessus de l'AP courant (mesurés par le même scan). Évite les allers-retours entre deux AP de niveau proche.<br>_Plage : 1 - 40_ |
...
| `Allow_Res_Heat_ChangeOver_COOL` | `boolean` | rw | false | Mi-saison : autorise le mode HEAT par la résistance électrique seule quand le changeover effectif (`ChangeOver_HVAC_Mode_Value`) est COOL, sur `FAN_COIL_2T_MIXED_2WIRES` et `FAN_COIL_2T_COOL_ONLY_2WIRES` uniquement. Faux (défaut) : comportement strictement inchangé, HEAT n'est pas proposé sous changeover COOL. Vrai : HEAT devient sélectionnable (écran, MQTT, HTTP, Modbus) dès que la résistance est disponible et non verrouillée (`Resistance_Lockout`) ; la vanne reste fermée (l'eau est froide) et la résistance travaille comme en `RESISTANCE_USAGE_PARALLEL`, quel que soit `Resistance_Usage_Type` — voir `Resistance_Usage_Effective`. Jamais AUTO. Pré-requis : un slot `HVAC_MODE_HEAT` dans `HVAC_Mode_List`, sinon le mode n'est pas sélectionnable (règle commune à tous les modes). Repasser à faux ramène le comportement d'origine à l'itération suivante, post-ventilation respectée.<br>_Absente du build MINIMAL._ |
